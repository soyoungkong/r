...
##### Convolution Operation with MNIST #####
...
# 모형이 작음에도 불구하고 CNN을 사용해서 0.989 퍼센트가 됨 


##### Convolution Operation without Max-pooling -  MNIST #####

# build model without max-pooling 
inputs <- layer_input(shape = c(28, 28, 1))
...
model_no_max_pool <- keras_model(inputs = inputs, outputs = outputs) #70만개. 맥스플랜을 해서 cnn을 줄여가야지 좋음 
model_no_max_pool
...
# test model
result_no_max_pool <- evaluate(model_no_max_pool, test_images, test_labels)
cat("Test accuracy:", result_no_max_pool['accuracy'], "\n")
...
#library(fs)
#dir_create("~/.kaggle")
#file_move("~/Downloads/kaggle.json", "~/.kaggle/") #캐글 가입 
...
path[1] <-"C:/workspace/github/r/04DeeLearning/data/dogs-vs-cats.zip"
path[1]
## ---- include = FALSE-----------------------------------------------------
...
# data sampling -> make a small dataset
...
# make tensorflow dataset (serialize)
library(tfdatasets)
...
##### Convnet with small dataset and data augmentation #####
...
##### Leveraging a pretrained model - feature extraction : vgg16 #####
...
# compare freeze weights vs. unfreeze weights : weights for fine-tuning 
...
##### Leveraging a pretrained model - fine-tuning : vgg16 #####
