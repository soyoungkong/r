...
# 감마 변경 
obj.radial            <- tune.svm(as.factor(SGG) ~., data=train_df, kernel="radial", 
                                  gamma=seq(0.1, 1, 0.1), cost=2^(-7:7))  
obj.radial$best.model  # tuning 한 best model 출력
...
plot(obj.radial$best.model, test_df, PRICE_GEN ~ LAND_ACCESS) #c가 좀 줄어듬 
plot(obj.radial$best.model, test_df, PRICE_GEN ~ UNIT_NUM) #c가 좀 늘어남
